    ...
    for (var curObj of property) {
        ...
        if ((curObj.flags.isPublic || isExported) && !curObj.flags.isProtected) {
            ...
            if (kString === 'Method') {
                ...
                content += '\n* `' + curObj.name + '`' + ' - ' + curObj.comment.shortText;
...
function getType(type, isEvent, curName) {
    ...
    if (!isUnion && iType && iType.indexOf('[') === -1) {
        if (type.type === 'reference' && !type.id && config.dependentAPI && config.dependentAPI[curModuleName] && config.dependentAPI[curModuleName][iType]) {
            iType = ' [`' + iType + '`](../' + config.dependentAPI[curModuleName][iType] + ')';
        } else {
            iType = ' `' + iType + '`';
        }
    }
...
function getReflectionType(obj) {
    ...
    if (obj.type === 'reflection' && obj.declaration.indexSignature) {
        if (obj.declaration.indexSignature.parameters) {
            for (let types of obj.declaration.indexSignature.parameters) {
                typeArray += typeArray.length > 0 ? ', ' : '';
                typeArray += types.name + ':' + (getType(types.type).replace(/`/g, ''));
                typeArray = '[' + typeArray + ']';
            }
        }
        typeArray = ' `{ ' + typeArray + ':' + getType(obj.declaration.indexSignature.type).replace(/`/g, '') + ' }`';
